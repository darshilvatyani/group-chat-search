...
## 🎬 Hackathon Presentation Walkthrough (2-Minute Demo Video Script)

1. **The Problem (0:00 - 0:30)**:
   - Type *"mountain vacation"* or *"Manali"* into standard search $\to$ show how searching for keywords returns dozens of irrelevant messages debating the trip, but misses the decision.
2. **The Zero-Word-Overlap Solution (0:30 - 1:00)**:
   - Click Query #2 from the Hard-8 tab: *"Who paid the apartment security deposit advance?"*
   - Show the Engine Inspector: **Route: Decision | Latency: 9ms**.
   - Show the match: Target #890 (*"token amount 25k transfer kar diya owner ko"*). Point out that **not a single word overlaps**!
3. **The Context Window (1:00 - 1:30)**:
   - Show how the interface renders the $\pm 4$ surrounding messages, providing the complete story (the negotiation, the broker fees, the confirmation) with the target highlighted in warm amber.
4. **The Honest Accuracy Gap (1:30 - 2:00)**:
   - Click the **"Report"** button in the top bar.
   - Show the judges the live evaluation modal comparing the Warmup 32 (78.1%) vs Hard 8 (25.0%) and explain the **53.1% accuracy gap**.
